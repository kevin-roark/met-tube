...
var previewFactor = 4;
var speed = 100; // higher is slower (upside down smiley face)
var skip = 0;
...
raster.onLoad = function () {
    ...
    // var rect2 = new Rectangle(500, 200, length, length);
    // var sub = raster.getSubRaster(rect2);
    // sub.fitBounds(view.bounds, true);
...
function onFrame (event) {
    ...
    if (event.count % speed === 0) {
        var colorIndex = ((event.count * (skip + 1)) / speed) % subRasters.length;
...
function getColors (r) {
    ...
    var colors = [];
    var subRasterArray = [];
    for (var y = 0; y < view.size.height - captureHeight; y += captureHeight) {
        for (var x = 0; x < view.size.width - captureWidth; x += captureWidth) {
